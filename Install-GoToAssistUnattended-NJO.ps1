...
if (-not $Location -or $Location -eq "") {
    Write-Error "Location parameter is required. Please specify a valid location."
    exit 1
}else if (-not $OU -or $OU -eq "") {
    Write-Error "OU parameter is required. Please specify a valid OU."
    exit 1
}
...
if ($RetryList -and (Test-Path $RetryList)) {
    ...
    $computers = Get-ADComputer -SearchBase "OU=Computers,OU=New Jersey,OU=North East,OU=Offices,DC=tcco,DC=org" -Filter {Enabled -eq $True} | Select-Object -ExpandProperty Name
}

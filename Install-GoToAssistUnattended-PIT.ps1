...
# Output paths
$logSuccess = "C:\Temp\InstallLogs\GoToAssist_Install_Success-PIT.log"
$logFailure = "C:\Temp\InstallLogs\GoToAssist_Install_Failures-PIT.log"
...
if ($RetryList -and (Test-Path $RetryList)) {
    ...
    $computers = Get-ADComputer -SearchBase "OU=Computers,OU=Pittsburgh,OU=North Central,OU=Offices,DC=tcco,DC=org" -Filter {Enabled -eq $True} | Select-Object -ExpandProperty Name
}

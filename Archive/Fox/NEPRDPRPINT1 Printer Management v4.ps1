...
Write-Host $wpf
	...
	Write-Host $WPFckb.Children

	$box = $WPFckb.Items
	Write-Host $box.IsChecked

	#Write-Host $wpfNEPPRINTER12FacilitiesColor.ischecked


foreach ($printer1 in $printers1) {


if($printer1.tf1 -eq $true) {write-host $printer1.name "true"}

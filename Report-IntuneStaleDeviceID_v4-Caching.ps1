...
Measure-Command {
    ...
    # Fetch all Intune devices and store in memory
    $IntuneDevices = Get-MGDevice -all
    $FilteredIntuneDevices = $IntuneDevices | Where-object {$_.OperatingSystem -eq "Windows"} | Select DisplayName, DeviceID
    $IntuneDeviceLookup = @{}
        foreach ($Device in $FilteredIntuneDevices) {
            $IntuneDeviceLookup[$Device.DisplayName] = $Device.DeviceID
        }
#Swap out the Get-MGDevice with the existing list and then do a spot compare. This will be faster than querying the graph

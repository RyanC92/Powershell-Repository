...
$WPFCombobox_building.Add_DropDownClosed({
	...
	<#$printer = Get-Printer | Where { $_.Location -like "$cob*"}
	$printer.Location#>
	$global:printers = import-csv  C:\Powershell-Repository\Fox\PrinterExport.csv | Where { ($_.Location -like "*$cob*") -or ($_.Comment -like "*$cob*")} | Sort-Object Sharename
...
Show-Form
	...
	$test2 = $WPFckb.Items.Count
	Write-Host "$test2"

	$test = $WPFckb.Items.IsChecked
	Write-Host "$test"
	#Write-Host "You Selected: $global:cob"


foreach ($printer in $global:printers) {


  $printer1 = $printer.name -replace '[^a-zA-Z0-9]',''


	$CheckBox1 = $form.FindName("NEPPRINTER12FacilitiesColor")

	Write-Host $CheckBox1

if($test -eq $true) {write-host $printer.name "true"}

...
$WPFCombobox_building.Add_DropDownClosed({
	...
	if ($cob -eq "Xerox") {
		...
		$printers = Get-Printer -Computername NEPPRDPRINT1 | Where { $_.Comment -like "*$cob*"} | sort-object Sharename
		...
		$printers = Get-Printer -Computername NEPPRDPRINT1 | Where { $_.Location -like "$cob*"} | sort-object Sharename

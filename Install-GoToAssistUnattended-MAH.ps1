...
# Output paths
$logSuccess = "C:\Temp\InstallLogs\GoToAssist_Install_Success-MAH.log"
$logFailure = "C:\Temp\InstallLogs\GoToAssist_Install_Failures-MAH.log"
...
if ($RetryList -and (Test-Path $RetryList)) {
    ...
    $computers = Get-ADComputer -SearchBase "OU=Computers,OU=Mahwah,OU=North East,OU=Offices,DC=tcco,DC=org" -Filter {Enabled -eq $True} | Select-Object -ExpandProperty Name
}
